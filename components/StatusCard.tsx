...
const StatusCard: React.FC<Props> = ({ user, status, onUpdateStatus, recommendation, dateLabel }) => {
  ...
  const getSeverityStyle = (severity?: string) => {
    switch(severity) {
      ...
      case 'moderate': return { bg: 'bg-amber-50 dark:bg-amber-500/10', border: 'border-amber-200 dark:border-amber-500/30', text: 'text-amber-700 dark:text-amber-200', icon: Lightbulb, iconColor: 'text-amber-500' };
      case 'safe': return { bg: 'bg-green-50 dark:bg-green-500/10', border: 'border-green-200 dark:border-green-500/30', text: 'text-green-700 dark:text-green-200', icon: PartyPopper, iconColor: 'text-green-500' };
      default: return { bg: 'bg-zinc-100 dark:bg-zinc-800', border: 'border-zinc-200 dark:border-zinc-700', text: 'text-zinc-600 dark:text-zinc-400', icon: Lightbulb, iconColor: 'text-zinc-400' };
    }
  };
  ...
  return (
    <div className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md rounded-3xl p-6 md:p-8 border border-primary-100 dark:border-zinc-800 shadow-sm space-y-6 transition-colors duration-300">
      ...
      {recommendation ? (
        <div className={`p-5 rounded-2xl border flex gap-4 ${style.bg} ${style.border} animate-fade-in relative overflow-hidden transition-colors`}>
           ...
           <div className="relative z-10">
             ...
             <p className={`font-bold text-lg md:text-xl mb-1 text-zinc-900 dark:text-white`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <button
          ...
          className={`group flex items-center justify-center gap-4 sm:flex-col sm:gap-4 p-4 md:p-6 rounded-2xl border-2 transition-all duration-200 ${
            status === 'GOING' 
              ...
              : 'border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 text-zinc-500 dark:text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:text-zinc-700 dark:hover:text-zinc-300 hover:border-zinc-300 dark:hover:border-zinc-700'
          }`}
        >
          <div className={`p-3 rounded-full transition-transform group-hover:scale-110 ${status === 'GOING' ? 'bg-green-500 dark:bg-green-600 text-white shadow-lg shadow-green-500/30 dark:shadow-green-900/50' : 'bg-zinc-200 dark:bg-zinc-800'}`}>
        ...
        >
          <div className={`p-3 rounded-full transition-transform group-hover:scale-110 ${status === 'NOT_GOING' ? 'bg-red-500 dark:bg-red-600 text-white shadow-lg shadow-red-500/30 dark:shadow-red-900/50' : 'bg-zinc-200 dark:bg-zinc-800'}`}>
        ...
        >
          <div className={`p-3 rounded-full transition-transform group-hover:scale-110 ${status === 'UNDECIDED' ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/30 dark:shadow-amber-900/50' : 'bg-zinc-200 dark:bg-zinc-800'}`}>
